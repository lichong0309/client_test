...
struct Graph {
    ...
    void forward(bool record_events, cudaEvent_t  *events) {
        for(int i = 0; i < num_stages; i++) {                          // num_stages ; stages的数量
            int stage_size = stage_num_seq[i];                      // stage_size : 每个stages的group的数量
            for (int j = 0; j < stage_size; j++) {                                         
                int seq_length = stage_seq_num_op[i][j];                    // seq_length:  每个group中操作的数量
...
DLL void graph_inference(const char *graph_json, int batch_size, data_type *input,
    ...
    for(int i = 0; i < num_convs; i++)
        graph.set_conv_weights(conv_names[i], filter_data[i], bias_data[i]);                // 为每个conv设置权重和bias,做inference

    graph.forward(false, 0);                // inference 前向传播
